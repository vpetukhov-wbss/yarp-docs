...
lede: >-
  As an ASP.NET Core component, YARP can easily integrate into different tracing systems the
...
YARP supports distributed tracing using Open Telemetry (OTEL). When a request comes in, and there is a listener for Activities, then ASP.NET Core will propagate the Trace Context trace-id, or create one if necessary, and create new spans/activities for the work performed. In addition YARP can create activities for:

Forwarding Requests Active health checks for clusters

These will only be created if there is a listener for the ActivitySource named Yarp.ReverseProxy .

Example: Application Insights

For example, to monitor the traces with Application Insights, the proxy application needs to use the Open Telemetry and Azure Monitor SDKs.
...
## Using custom tracing headers {#using-custom-tracing-headers}
...
## Pass-through proxy {#pass-through-proxy}
...
## DistributedContextPropagator DistributedContextPropagator.Fields {#distributedcontextpropagator-distributedcontextpropagator-fi}
